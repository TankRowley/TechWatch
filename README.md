...
- ベンダー・OSS・独立メディア・コミュニティ・研究の20個のRSS/Atom情報源から並列収集
- SQLiteへ冪等保存し、同一URLを重複登録しない
- タイトル・概要から登録キーワードを抽出
- 情報源の信頼度、キーワード重み、Core/Buzzを使って記事を評価
- `Core / Watch / Buzz / Decline / Ignore`でキーワードを自動評価
- 完了した月曜〜日曜の取得記事数で補正した言及率を4・13・52週で比較
- 初回実行時にRSSが返した過去記事を最大52週まで`HISTORICAL_PARTIAL`として自動集計
- 履歴取得は1情報源250件までとし、14日より古い記事は本文取得・AI要約を行わず統計だけ作成
...
- JDK 21以上（EXE生成に使うため、JREではなくJDKが必要です）
- インターネット接続（RSSおよび本文取得用）

Mavenは同梱のMaven Wrapperが自動取得するため、別途インストールする必要はありません。

OpenAI APIやLM Studioは任意です。設定しなくても、記事収集、ルールベース評価、履歴、求人CSV、週報、GUIを利用できます。

## 最短で起動する

1. [TechWatchのGitHubページ](https://github.com/TankRowley/TechWatch)からクローンするか、Code → Download ZIPで取得します。
2. ZIPの場合は展開します。
3. フォルダー直下の`start-techwatch.cmd`をダブルクリックします。

初回だけテスト、ビルド、EXE生成を自動で行うため数分かかります。生成後はアプリが自動で起動します。2回目以降は同じファイルをダブルクリックすると、生成済みのEXEをすぐ起動します。

生成されるEXEは`dist/てっくにゅーす/てっくにゅーす.exe`です。強制的に再ビルドする場合はPowerShellで次を実行します。

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\setup-and-run.ps1 -Rebuild
```

## GitHubからコマンドでクローンして使う

PowerShellを開き、次の順番で実行します。初回の起動ではMavenが必要なファイルをダウンロードするため、完了まで少し時間がかかることがあります。

```powershell
# リポジトリを取得して、プロジェクトのフォルダへ移動
...
2回目以降も`start-techwatch.cmd`を実行するだけです。アプリを終了するときはGUIを閉じてください。

初回起動時にローカルDBと週報フォルダが自動作成され、初回セットアップ画面が表示されます。APIキー、DB、実行ログ、生成した週報、メール送信先はGitへ追加されません。
...
`sources.yml`でRSS/Atom情報源、信頼度（1〜5）、情報源分類を編集します。
...
`keywords.yml`で初期ステータス、記事評価用の重み、別名を編集します。
...
配布版では設定画面に表示される保存先の`config\job-market.csv`を編集し、週報を生成すると履歴・市場評価・グラフへ反映されます。件数は検索条件やデータ元で変わるため、絶対値ではなく傾向比較として扱います。CSVが未更新の週は0件として保存しません。
...
java -jar target/techwatch.jar backtest
```
...
デスクトップから起動する場合は、`てっくにゅーす.exe`を右クリックし、「その他のオプションを確認」→「送る」→「デスクトップ（ショートカットを作成）」を選びます。`exe`だけを別の場所へ移動せず、`dist/てっくにゅーす/`フォルダはまとめて残してください。
...
app-image版で毎週月曜8時の自動収集を登録する場合は、配布フォルダーで次を実行します。
